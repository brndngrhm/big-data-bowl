...
model_data_prep <- 
  ...
  select(-c(play_description, down, yards_to_go, game_clock, possession_team, special_teams_play_type, special_teams_result, kicker_id, returner_id, kick_blocker_id, 
            yardline_side, yardline_number, starts_with("penalty"), starts_with("pre"), pass_result, kick_length, kick_return_yardage, play_result, 
            ...
            teams, recieving_team, recieving_team_name, kicking_team_name, epa, drive_start_yard_line)) %>% 
  select(game_id, play_id, return_type, everything())

factor_cols <- 
  c("quarter", "season", "game_hour", "game_tod", "primetime_ind", 
    "overseas_game_ind", "holiday", "home_team_recieving_ind", "home_timeouts_remaining",
